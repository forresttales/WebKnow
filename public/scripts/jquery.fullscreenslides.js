...
(function($) {
	...
	var attachEvents = function() {
			...
			if (!isLoading) {
				...
				if (!("$img" in newSlide)) {
					...
					.load(function() {
						...
						isLoading = false;
						newSlide.error = true;
						$container.trigger("stopLoading").trigger("error", newSlide);
		...
		$container.bind("prevSlide nextSlide", function(event) {
			var nextID, slides = $container.data("slides"), currentSlide = $container.data("currentSlide"), currentID = currentSlide && currentSlide.id || 0;
	...
	$.fn.fullscreenslides = function(options) {
		...
		options.useFullScreen = options.useFullScreen && !!($container[0].requestFullScreen || $container[0].mozRequestFullScreen || $container[0].webkitRequestFullScreen);
