class UserListsController < ApplicationController
  ...
  def add_user_friend
      ...
      if params[:user_friend_id].present?
          @new_user_friend = User.find_by_id(params[:user_friend_id].to_i)
          current_user.user_friendships.request(current_user, @new_user_friend)
      end
  ...
  def remove_user_friend
      ...
      if params[:user_friend_id].present?
          @old_user_friend = User.find_by_id(params[:user_friend_id].to_i)
          @result = UserFriendship.remove(current_user, @old_user_friend)
      end
  ...
  def accept_user_friend

    if params[:user_friend_id].present?
      @new_user_friend = User.find_by_id(params[:user_friend_id].to_i)
      @result = UserFriendship.accept(current_user, @new_user_friend)
    end
